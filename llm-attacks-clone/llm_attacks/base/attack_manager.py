...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        elif self.conv_template.name == 'raw': #for raw template
            self.conv_template.messages = []
        ...
        elif self.conv_template.name == 'alpaca': #for llama-it
            self.conv_template.messages = []
            self.conv_template.append_message(self.conv_template.roles[0], None)
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._user_role_slice = slice(None, len(toks))
    ...
    def test(self, model, gen_config=None):
        ...
        jailbroken = any([prefix in gen_str for prefix in self.test_prefixes])
...
def get_goals_and_targets(params):
    ...
    if params.train_data:
        ...
        train_data = train_data['prompt']
        random.shuffle(train_data)
        train_targets = ['Purple'] * params.n_train_data
        train_goals = train_data[offset:offset+params.n_train_data]

        if params.n_test_data > 0:
            test_targets = ['Purple'] * params.n_train_data
            test_goals = train_data[offset+params.n_train_data:offset+params.n_train_data+params.n_test_data]
